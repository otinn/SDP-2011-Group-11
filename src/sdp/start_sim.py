...
if __name__ == "__main__":
    ...
    SynchronizedPrint.important.put_nowait(common.utility.current_time()[11:-3] + ' Waiting for the knowledge server to start')
    while not knowledge_server.ready:
        time.sleep(0.001)
    SynchronizedPrint.important.put_nowait(common.utility.current_time()[11:-3] + ' Knowledge server started')
    ...
    modules = {
        ...
        'robot': ['python proxy.py --host ' + options.host + ' -p ' + str(options.port) + ' ./simulator/simulator' + ('.exe' if os.name == 'nt' else ''), None],
    }
